...
if __name__ == "__main__":
    ...
    for DATASET_NAME in DATASET_LIST:
        ...
        clf = MLPClassifier(hidden_layer_sizes=([100]), activation='relu', learning_rate_init=0.001, max_iter=200, random_state=1).fit(trainX, trainY)
        score = clf.score(testX , testY)
        print('Score MLP:', score)

        linear = PerceptronClassifier(learning_rate=0.001, max_iter=200)
        linear.fit(trainX, trainY)
        linear_score = linear.score(testX, testY)
        print('Score Perceptron:', linear_score)
